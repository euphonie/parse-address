...
(function(){
  ...
  function lazyInit(){
    ...
    Addr_Match = {
      ...
      zip     : '(?<fsa>[A-Za-z][0-9][A-Za-z])[ ]?(?<ldu>[0-9][A-Za-z][0-9])?',
      corner  : '(?:\\band\\b|\\bat\\b|&|\\@)',
    };

    Addr_Match.number = '(?<unit_number>(\\d+)|([N|S|E|W]\\d{1,3}[N|S|E|W]\\d{1,6}))(?=\\D)(-)?(?<civic_number>(\\d+)?)';
  ...
  parser.normalize_address = function(parts){
    ...
    Object.keys(parts).forEach(function(k){
      ...
      if(parts[k])
        parsed[key] = parts[k].trim().replace(/^\s+|\s+$/g, '');
    });
